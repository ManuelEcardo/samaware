...
import 'package:samaware_flutter/modules/Scanner/ScannerOrderDetails/ScannerOrderDetails.dart';
import 'package:samaware_flutter/shared/components/Imports/default_imports.dart';

class ScannerOrdersSettings extends StatefulWidget {
  const ScannerOrdersSettings({super.key});

  @override
  State<ScannerOrdersSettings> createState() => _ScannerOrdersSettingsState();
}

class _ScannerOrdersSettingsState extends State<ScannerOrdersSettings> {
                                ...
                                [
                                  ...
                                  Expanded(
                                    child: ListView.separated(
                                      ...
                                      shrinkWrap: true,
                                      itemBuilder: (context,index)=>itemBuilder(cubit: cubit, context: context, order: cubit.allScannerOrders?.orders?[index]),
                                      separatorBuilder: (context,index)=> const SizedBox(height: 20,),
                                      itemCount: cubit.allScannerOrders!.orders!.length,
                                    ),
                                  ),

                                  if(state is AppGetNextScannerOrdersSLoadingState && cubit.allScannerOrders?.orders?.length !=0)
  ...
  Widget itemBuilder({required AppCubit cubit, required BuildContext context, required OrderModel? order})
  {
    return defaultBox(
      ...
      onTap: ()
      {
        navigateTo(context, ScannerOrderDetails(order: order!));
      },
      boxColor: null,
  ...
  void _onScroll(AppCubit cubit)
  {
    ...
    {
      if(cubit.allScannerOrders?.pagination?.nextPage !=null)
      {
        print('paginating next scanner orders...');
        cubit.getNextWorkerOrdersScanner( nextPage: cubit.allScannerOrders?.pagination?.nextPage);
      }
